...
- **Optional Comments:** [giscus](https://giscus.app) (`src/components/GiscusComments.tsx`) embeds threaded GitHub Discussions-backed comments on Encyclopedia/Standards Explorer/Security Bulletins entries — same env-gated pattern as Drive Backup (§4DD), gated behind `VITE_GISCUS_REPO`/`VITE_GISCUS_REPO_ID`/`VITE_GISCUS_CATEGORY`/`VITE_GISCUS_CATEGORY_ID`.
...
| **`/learn`** | `Learn.tsx` | IAM Academy. 7 tracks, 42 expandable modules with local progress bar persistent tracking. |
...
| **`/career-center`** | `InterviewCareerCenter.tsx` | Comprehensive role-based interview preparation system spanning 6 role tracks featuring MCQs, scenarios, design simulations, coding terminals, timed mocks, and resume guidelines. Its Resume & Portfolio tab embeds `PortfolioExport.tsx`, which reads real `localStorage` progress to auto-draft resume bullets, award a self-baked Open Badges 2.0 SVG badge, export a Markdown/print-to-PDF portfolio, and download a Web Crypto-signed JSON completion certificate (`src/lib/career/certificateSigner.ts`, ECDSA P-256, checkable at `/tools/certificate-verifier`). Both surfaces carry the same mandatory honesty caveat: because the signing key ships inside this client-only app's own bundle, a valid signature proves the certificate is internally self-consistent — not that it's cryptographically unforgeable or a substitute for third-party professional certification. |
...
| **`/tools/cyber-insurance-readiness`** | `Tools/CyberInsuranceReadiness.tsx` | 6-control checklist (`src/lib/tools/insuranceReadiness.ts`) insurers commonly underwrite against — phishing-resistant MFA (privileged/remote/email), PAM governance, Zero Trust posture, immutable backups — each contributing fixed points to a purely additive 0-100 score (strictly monotonic: enabling a control can never lower it), mapped to one of 4 directional premium-impact bands. Ships 2 real, cited cyber-insurance coverage-denial cases (`INSURANCE_DENIAL_CASE_STUDIES`) tied specifically to inadequate/misrepresented MFA, each with a source link and a "last verified" date. |
...
| **`/assess`** | `Assess.tsx` | GRC Maturity Wizard. Self-assessments with dynamic charts, downloadable SVG roadmaps, and a `?a=<digits>` shareable, URL-hydrated read-only report link (scoring logic lives in `src/lib/assess/scoring.ts`). The results view also maps the score to a 5-level industry maturity model and an estimated peer percentile (`src/lib/assess/maturityBenchmark.ts`), and carries the `JourneyBreadcrumb` (`src/data/executiveJourneySteps.ts` is the single source of truth for the step sequence). On completing the wizard (not on a shared `?a=` report view), `saveLastAssessment()` (`src/lib/assess/assessHistory.ts`) persists the answers to `localStorage` so `/command-center` can reuse them. |
| **`/command-center`** | `CommandCenter.tsx` | Executive Command Center. A curated hub, not a new assessment — 4 cards link to `/assess`, `/standards?view=deadlines`, `/tools/iam-tco-calculator`, `/tools/raci-builder`. "Generate Board Summary" (`src/lib/assess/boardSummary.ts`) reads the persisted last-assessment answers via `getLastAssessment()`, builds a dollar-exposure-framed one-page narrative naming the weakest pillar(s) (ties included), and appends a fixed NHI-governance addendum regardless of pillar scores; renders an empty-state prompt instead if no assessment is on file. The Density toggle calls `usePreferenceStore`'s existing `setDepthMode()` (`'beginner'`/`'expert'`) rather than a new preference. First step in `EXECUTIVE_JOURNEY_STEPS`. |
...
| **`/playground/gaming-identity`** | `Playgrounds/GamingIdentityLab.tsx` | Gaming & esports identity — an untouched vertical vs. every other lab's enterprise/workforce/consumer focus. 3 scenarios backed by `src/data/gamingIdentityScenarios.ts`'s `evaluateGamingScenario()` (a deterministic, pure signal-set → outcome evaluator, no randomness): cross-platform account-linking ban propagation, a weighted-confidence smurf/ban-evasion detector (`SMURF_SIGNAL_WEIGHTS`, threshold 55), and continuous wagering KYC (any single risk signal re-triggers verification). Uses the standard `usePlayground`/`PlaygroundShell`/`TraceTerminal` SDK. |
| **`/playground/stix-taxii-ioc`** | `Playgrounds/StixTaxiiIocLab.tsx` | Teaches the STIX 2.1 object model and TAXII 2.1 collection/subscription exchange *format*, not detection logic — distinct from the CAEP Event Storm Visualizer (session-state propagation) and the ITDR Lab (log monitoring). `src/data/stixTaxiiScenarios.ts`'s `buildStixBundle()` assembles a minimal Indicator + related Malware/Threat-Actor/Identity SDO + Relationship object bundle (viewable as raw JSON via "View Raw STIX JSON"); `matchesSubscriberFilter()` fans it out to 3 mock TAXII subscribers by tag overlap, reusing the CAEP visualizer's static hub-and-spoke SVG diagram convention for the fan-out shape even though the payload/protocol is unrelated. |
...
To add a new entry: confirm the advisory genuinely doesn't apply to this app's actual usage (not just "it's inconvenient right now"), then add `{ ghsaId, package, reason, reviewBy }` to `ALLOWLIST`. When an allow-listed advisory does get fixed upstream, delete its entry in the same commit as the bump rather than leaving it to expire. Prefer fixing the real vulnerability (`npm audit fix`, or `npm audit fix --force` after checking the breaking change is safe) over allow-listing whenever a fix is actually available — the allow-list is for the specific case for a fix doesn't exist yet.
...
**giscus (`src/lib/giscusConfig.ts` + `src/components/GiscusComments.tsx`) is the second feature built on this exact pattern**, with one deliberate difference worth calling out: unlike Drive Backup (an opt-in personal utility a visitor has to go looking for), comments render by default on every Encyclopedia term, Standards Explorer entry, and Security Bulletin — visible to every visitor, not just ones who seek it out. That distinction is why this was the one Phase 3 item that got an explicit product go/no-go check with the user before building, rather than being built by default like every other item in that backlog. `getGiscusConfig()` requires all 4 `VITE_GISCUS_*` values together (any single one missing returns `null`, same as Drive Backup's single-value check) and `GiscusComments.tsx` injects the actual `https://giscus.app/client.js` embed script imperatively inside a `useEffect` (a `data-*`-attributed `<script>` tag, matching giscus's own documented embed contract) rather than rendering giscus's own custom element directly — this keeps the component itself trivially unit-testable (assert the container div and its script child exist, dispatch a synthetic `error` event to verify the network-blocked fallback message, per `GiscusComments.test.tsx`) without needing to mock the real third-party script's runtime behavior.
