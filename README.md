...
3. **Zero Backend, Complete Privacy:** 100% of cryptography (Web Crypto HMAC), policy evaluations, and data reviews run strictly in-device, ensuring zero server costs and ultimate user privacy. Even the optional Google Drive Backup & Restore feature honors this: the browser talks directly to Google's APIs using the user's own storage, with no AboutIAM server ever in the loop.
...
* **Daily Identity Puzzle (`/daily-puzzle`):** A new, date-seeded identity-security puzzle every day — spot the JWT vulnerability, catch the tampered SAML field, or guess the protocol from progressively revealing clues — so every visitor gets the same challenge on the same day. Up to 3 attempts, a shareable Wordle-style emoji result grid, and a compact `?r=<code>` share link that replays a friend's result. Featured as a Home dashboard widget.
* **Optional Google Drive Backup & Restore:** A privacy-first, opt-in sync of locally-persisted progress (Academy completion, bookmarks, badges, personalization) to a hidden folder in the user's own Google Drive (`drive.appdata` scope only) — the browser talks directly to Google's APIs, no AboutIAM server is ever involved, and the access token is kept in memory only. Inert until a deployment configures `VITE_GOOGLE_CLIENT_ID`; see `/terms#cloud-sync-privacy` for the full privacy breakdown.
* **Offline Study Pack:** One-click download of a `.zip` bundling the Encyclopedia, Living Standards, Architecture Center, and Developer Playbooks as plain Markdown — for offline review or feeding into a personal notes app/AI tool. Built entirely client-side (JSZip is lazy-loaded on click so it never bloats the initial page load).
...
* **Knowledge Graph (`/knowledge-graph`):** A visual map connecting Standards, Encyclopedia terms, and Architecture Center entries by hand-curated relationship — search or click a concept to see its direct neighbors and jump straight to the matching page. A concentric-ring SVG overview (Standards → Terms → Architectures) pairs with a searchable, type-filterable list and a detail panel, so the graph stays fully usable at any screen size even without precise taps on small SVG nodes.
...
* **Identity Research & CVE Tracker (`/research`):** Beginner-to-advanced identity CVE directory (13 entries — Log4Shell, Spring4Shell, Zerologon, Shellshock, JWT `alg:none`, SAML signature wrapping, and more) with side-by-side vulnerable vs. secure code-remediation patches, plus a 17-entry IETF RFC/draft protocol registry (OAuth 2.0/2.1, JWT, PKCE, SCIM, DPoP, GNAP, Kerberos, LDAP, TOTP). Both panels are difficulty-filterable and every entry is individually searchable and deep-linkable via `?cve=<id>`/`?rfc=<slug>`.
...
* **Interview & Career Preparation Center (`/career-center`):** Comprehensive role-based interview preparation system. Spans 6 role tracks (Fresher -> Principal) featuring interactive multiple-choice verification, outage incident walkthroughs, real-time system design audits, config code terminals (LDAP filters, OPA Rego rules), a scripted **CLI Muscle-Memory Practice terminal** (`openssl x509`, `curl`-mint-a-token, `kinit`, `jwt-cli decode` against fabricated IAM infrastructure — closer to real muscle-memory CLI practice than a static snippet), timed mock interviews, and copyable high-impact resume bullets. The Resume & Portfolio tab also includes a **Dynamic Portfolio Builder** that reads your actual Academy/Playground progress from `localStorage` to auto-draft quantified resume bullets, awards a self-contained, verifiable Open Badges 2.0 SVG badge at 20+ completed modules, and exports the whole portfolio as Markdown/print-to-PDF — nothing is ever fabricated.
* **Security Bulletins & IR Simulator (`/bulletins`):** 18 beginner-to-advanced identity incident bulletins — spanning Credential & Session Theft, MFA & Push Fatigue, Federation & SSO Exploits, OAuth & Token Abuse, Cloud IAM Misconfiguration, Directory & Kerberos Attacks, and Supply Chain & Provisioning (Okta support HAR cookie theft, SolarWinds Golden SAML, Kerberoasting, CI/CD supply-chain compromise, cross-tenant federation forgery, and more) — difficulty/category filterable, individually searchable and deep-linkable via `?bulletin=<id>`, each paired with a data-driven "Crisis Response Console" simulation game. The same 18 bulletins also power the standalone Tabletop Exercise Generator tool for live, in-person team exercises.
...
* **AI Knowledge Assistant 2.0 (`/assistant`):** Four-tab platform navigator — a Knowledge Chat with highly context-aware, automatic tool/playground recommendations across 45+ IAM topics; a Comparison Engine with 20 side-by-side protocol/product pairings (OAuth vs OIDC, SAML vs OIDC, PAM vs IGA, JWT vs PASETO, GNAP vs OAuth 2.0, and more); a Learning Planner with 8 beginner-to-expert career roadmaps (Security Engineer / IAM Architect × Beginner/Intermediate/Advanced/Expert); and an Interview Prep tab with 16 domain-filterable mock interview questions and reveal-to-check answers. Every comparison, learning track, and interview question is individually searchable and deep-linkable (`/assistant?tab=compare&compare=<id>`, `?tab=learn&level=<lvl>&goal=<goal>`, `?tab=interview&q=<id>`).
...
* **mTLS & Certificate Chain Validator:** Visual hierarchical map of Certificate Authorities (Root -> Intermediate -> Leaf) allowing users to simulate CRL/OCSP revocations and trace mTLS handshakes. A Classical/Hybrid PQC toggle swaps the per-hop signature algorithm and size annotation (~96B ECDSA vs. ~4.7KB hybrid ECDSA+ML-DSA-87) and shows a "Harvest Now, Decrypt Later" attack timeline that only applies in classical mode.
...
* **NHI Sprawl Cleanup Game:** Triage a seeded fleet of 60 (of a simulated 500) service accounts, API keys, and CI/CD tokens against a real non-human-identity governance rubric — rotate what's stale, revoke what's orphaned or over-privileged, and keep what's legitimately still in use, with a cascading-failure trace log when a revoke breaks a dependent service.
* **Passkey Fleet Rollout Strategist:** Play CISO — allocate a fixed 100-point rollout budget across platform SDKs, help-desk training, legacy-fallback sunset, and account-recovery investment, then watch four quarters of adoption, phishing-incident, and help-desk-ticket outcomes unfold under a deterministic, diminishing-returns model that rewards a balanced allocation over dumping the whole budget into one category, scored against real FIDO Alliance 2026 industry benchmarks.
* **IAM Modernization Backlog Game:** Sequence 20 realistic legacy-IAM tech-debt items (hardcoded LDAP binds, un-rotated service passwords, SAML-only SSO, no step-up MFA, and more) into a 12-month roadmap under a fixed quarterly budget, respecting dependency ordering and maximizing risk-reduction-per-dollar — the natural next step after the `/assess` GRC Maturity Wizard scores where you currently stand.
* **Build-Your-Own-IdP Sandbox:** A guided 5-step builder that assembles a minimal OIDC Provider — generate a real RS256 signing keypair via Web Crypto, configure the discovery document and JWKS, register a client, build a consent screen — then watch a mock Relying Party run the full authorization-code + PKCE flow against it and mint a real, self-verifying signed ID token, entirely offline. Deep-links the minted token to the JWT Decoder for further inspection.
* **OpenID4VC Wallet Studio:** Issue a real SD-JWT verifiable credential (each claim independently salted and hashed as its own disclosure) across 4 real-world scenarios (mDL age-check, university diploma, employment proof, pharmacy eligibility), store it in a mock wallet with per-claim include/exclude toggles, and selectively disclose only what a verifier actually requested — the OID4VCI/OID4VP flow behind eIDAS 2.0 EUDI Wallets — with the trace log calling out both missing and over-disclosed claims.
* **Trust Registry & Issuer Governance Explorer:** Reuses the OpenID4VC Wallet Studio's same 4 credential scenarios, but checks something different — not whether a credential's signature is valid, but whether its issuer is currently authorized in the specific national trust registry a verifier trusts. Demonstrates the real EUDI Wallet cross-border recognition gap (an issuer authorized in Germany isn't automatically trusted by a French verifier's registry) and a live revocation scenario where a previously-issued credential fails authorization the moment its issuer is revoked, even though the signature itself never changes.
* **Cloud Entitlement Graph Explorer (CIEM Lite):** Inventory hygiene of the permissions themselves, not credential lifecycle or SoD review. Click a role in a seeded AWS-style policy graph (reusing the Attack-Path Graph Visualizer's force-directed layout engine) to see toxic privilege-escalation combinations like `iam:PassRole` + `lambda:CreateFunction` — including one only reachable by chaining a cross-account role-assumption — toggle "granted" vs. "effective" permissions, and shrink a role to least privilege using a mock CloudTrail-style access log.
* **FAPI 2.0 / Open Banking Security Profile Playground:** Toggle each of the three controls FAPI 2.0 adds on top of plain OAuth 2.0 for financial-grade APIs — Pushed Authorization Requests, sender-constrained tokens (mTLS/DPoP), and signed authorization responses (JARM/JAR) — then simulate the matching attack with the control off (succeeds) and on (blocked), with both branches rendered side-by-side in the trace log.
* **CAEP Event Storm Visualizer:** Fire a Shared Signals Framework / CAEP event from a mock IdP Event Bus and watch it fan out live to 4 mock relying parties, each with its own subscription list, simulated latency, and enforcement decision — an unsubscribed relying party is explicitly marked as having ignored the event, and toggling the header's Airplane Mode simulates one relying party going offline, dropping delivery to it alone and exposing the real-world CAEP enforcement blind spot.
* **Identity Attack-Path Graph Visualizer:** A BloodHound-style force-directed graph (a custom spring/repulsion physics simulation, no charting dependency) over a seeded identity dataset of users, groups, service accounts, and machines — click hop-by-hop along real `MemberOf`/`AdminTo`/`HasSession`/`CanRDP`/`Owns` edges to trace a privilege-escalation path to Domain Admin or Cloud Admin, across a 10-node beginner scenario and a 24-node advanced scenario with a genuinely longer decoy route, then reveal the true shortest path with a real-world technique breakdown for every hop (Kerberoasting-style credential dumping, GenericAll ACL abuse, service-account reuse, and more).
...
* **Identity Fabric / Orchestration Flow Builder:** Wire a legacy protocol-only app to a modern protocol-only IdP through an Orchestration Node across 3 preset scenarios (IdP migration without an app rewrite, consistent MFA policy across heterogeneous IdPs, one node fronting both LDAP and OIDC apps) — a direct app-to-IdP wire that bypasses orchestration fails with a clear protocol-mismatch error, while a correct wiring plays back every translation step in order.
* **Liveness Detection & Injection Attack Lab:** Pick a face-verification attack (presentation replay, camera-feed injection bypassing the camera driver entirely, or a real-time face-swap deepfake) and a defense stack (single static photo, challenge-response flash sequence, passive depth/micro-movement analysis, or full ISO 30107-3 PAD scoring) and see exactly which defense catches which attack class and why — the same flash-challenge defense that defeats replay is bypassed outright by camera-feed injection, the decisive 2026 shift this lab is built to demonstrate.
* **OT/ICS Device Identity & Segmentation Simulator:** A factory-floor topology where most devices (PLCs, HMIs, sensors) structurally cannot authenticate — toggle between a flat network and identity-based microsegmentation, trigger a ransomware injection at a compromised HMI, and watch the reachable-node blast radius drop from every device on the network down to just its own local zone, alongside a real-world ~42-day vs. ~5-day dwell-time comparison.
* **Legacy & Academic Federation Playground:** Three tabs covering the "protocols before OAuth/SAML dominance" story — a RADIUS Access-Request/Access-Accept packet exchange (shared-secret mismatch produces a real Access-Reject), TACACS+'s separated Authentication/Authorization/Accounting phases contrasted against RADIUS's combined AAA (a privileged `configure terminal` command is denied separately from authentication), and a Shibboleth/eduGAIN WAYF ("Where Are You From") discovery-service picker that redirects to a chosen institution's home IdP and returns a SAML assertion the mock SP consumes.
...
* **Passwordless Magic Link & Step-Up Auth Lab:** Email magic-link login followed by a forced step-up to WebAuthn/OTP before a high-risk action.
* **Credential Stuffing & Password Spray Defense Lab:** Replays leaked credentials against a mock login and toggles rate-limiting, CAPTCHA, breached-password detection, and lockout defenses.
* **CIAM Consent & Progressive Profiling Sandbox:** Social login consent screen, OAuth scope grants, and progressive profile-field collection across sessions.
* **Access Certification Campaign Simulator:** Reviewer walks user-to-entitlement rows, approves or revokes access, and flags Separation-of-Duties (SoD) conflicts.
...
* **HR-to-IdP Attribute Mapper:** Click an HR field, then click a target identity-store attribute to connect them (no drag-and-drop) — apply concat/regex/lookup-table transformations, watch a live sample-record preview update, and get clear warnings for a duplicate-target mapping or an unmapped required attribute.
...
* **Access Request Cart Simulator:** Shop a mock entitlement catalog and submit a request through a deterministic approval chain — manager approval always runs first, a privileged entitlement additionally requires app-owner sign-off, and a Separation-of-Duties conflict against your cart or existing access triggers a compliance-officer override, blocking auto-approval.

### 🛠️ C. Security Tools (`/tools`)
* **Free, 100% client-side IAM/security utilities** — no signup, no uploads, nothing leaves the device. 40 tools are live:
  ...
  * **Program & Vendor Management:** IAM Build vs. Buy TCO Calculator (3-year cost projection comparing an in-house identity stack against a commercial IDaaS subscription, with an optional breach-risk-adjusted cost toggle), IAM RFP Generator (a short questionnaire produces a categorized, downloadable RFP with deep links to named vendors known to support each priority capability), IAM Salary Compass (a filterable, IAM-specific compensation comparator by role, seniority, specialization, and region — directional estimates aggregated from public sources), Tabletop Exercise Generator (turns a real Security Bulletins incident into a printable, facilitator-ready live-team exercise script with timed injects, discussion prompts, and a scoring rubric).
  * **Post-Quantum & Crypto-Agility:** PQC Readiness Auditor — paste a PEM certificate chain, a JWKS JSON blob, or a TLS cipher-suite list to flag quantum-vulnerable algorithms (RSA, ECDSA, EdDSA, AES-128) against a curated NIST FIPS 203/204/205-cited risk table, estimate hybrid post-quantum handshake size growth (a ~4.6KB ML-DSA-87 signature vs. a ~96B classical ECDSA one), and download a prioritized Critical/High/Medium/Info migration checklist as JSON.
...
* **Master A-Z Encyclopedia (`Encyclopedia.tsx`):** Searchable glossary packed with **182 standard identity terms**. Details analogies and strict technical specifications, with a "🔊 Listen" Read-Aloud button (Web Speech API, zero dependency) on every term for audio-first/low-vision/dyslexia-friendly learning.
* **Vulnerability Lab & Identity Museum (`WallOfShame.tsx`):** Interactive corridor charting the 5 historic eras of identity, plus a difficulty-filterable **Breach Archive** of 27 beginner-to-advanced real-world incidents (backed by `src/data/breachesData.ts`) spanning Federation & SAML, MFA & Push Bombing, OAuth & OIDC, Session & Token Theft, Directory & Kerberos, Privileged Access, Supply Chain, Cloud & SaaS Misconfiguration, Credential & Password Attacks, and Social Engineering & Help Desk. Six flagship cases (SolarWinds **Golden SAML**, **Silver SAML**, **MFA Fatigue push bombing**, Okta HAR cookie theft, OAuth wildcard redirects, LastPass vault cracking) carry detailed hand-built interactive steppers; every other entry gets a vulnerable-vs-secure code deconstruction panel. Each breach is individually searchable and deep-linkable via `?tab=breaches&lab=<id>`, and carries `ContentFeedback`/bookmark widgets. A **🧠 Quiz Mode** tab (`?tab=quiz`) turns every breach into an active-recall flashcard (front: title/attack vector, back: root cause/remediation) scheduled client-side with the SM-2 spaced-repetition algorithm (`src/lib/learning/spacedRepetition.ts`), persisted per-breach in `localStorage` — no extra authoring needed since it's 100% reused from the existing breach data.
* **Developer Playbooks (`CheatSheets.tsx`):** 24 beginner-to-advanced interactive checklists (backed by `src/data/cheatSheetsData.ts`) spanning **Application Security** (SPA/PKCE, M2M, password & session management, JWT, OAuth 2.0/OIDC, SAML, REST APIs, CIAM social login), **Identity Infrastructure & Governance** (secrets management, LDAP/AD hardening, IGA access reviews, Zero Trust, Kubernetes RBAC, Kerberos tiering, identity incident response), and **Compliance & Regulatory** frameworks (SOC 2 Type II, ISO 27001, HIPAA, PCI-DSS, NIST 800-63-3, GDPR, CCPA/CPRA, FedRAMP High, DORA), each driving a dynamic real-time compliance gauge. Difficulty-filterable, individually searchable, deep-linkable via `?sheet=<id>`, and carries `ContentFeedback`/bookmark widgets.
* **GRC Maturity Wizard (`Assess.tsx`):** 5-pillar questionnaire evaluating organizational readiness, plotting dynamic charts, exporting a downloadable vector SVG roadmap, and generating a shareable, URL-hydrated read-only report link. The results view maps your score to a 5-level industry maturity model (Initial → Transformational, original non-proprietary level summaries) and an estimated peer percentile against a static, cited directional dataset.
...
The site runs fully featured with **zero configuration** — no `.env` file is required. One feature is opt-in and inert until configured: copy `.env.example` to `.env.local` and fill in `VITE_GOOGLE_CLIENT_ID` to enable the Google Drive Backup & Restore card on the home page. See the comments in `.env.example` for how to create the OAuth Client ID.
