...
export function TraceTerminal({ 
  ...
  return (
    <div className={`rounded-2xl border border-border-subtle bg-[#030712] text-[#f9fafb] terminal-high-contrast font-mono text-xs p-4 ${height} flex flex-col shadow-inner relative overflow-hidden`}>
      <div className="flex items-center justify-between border-b border-border-subtle/30 pb-2 mb-3 text-[10px] uppercase font-bold text-slate-400 shrink-0 select-none">
      ...
      <div className="flex-grow overflow-y-auto space-y-2 pr-2 scrollbar-thin">
        {logs.length === 0 ? (
          <div className="text-slate-500 flex items-center gap-2 h-full justify-center text-center">
          ...
          logs.map((l, i) => (
            <div key={i} className="flex gap-2.5 items-start leading-relaxed animate-in fade-in slide-in-from-bottom-1 duration-150">
              <span className="text-slate-500 font-light shrink-0">[{l.timestamp}]</span>
              <span className={`font-semibold ${
                l.type === 'success' ? 'text-[#34d399]' :
                l.type === 'error' ? 'text-[#f87171]' :
                l.type === 'warning' ? 'text-[#fbbf24]' : 'text-[#e5e7eb]'
              }`}>
                {l.message}
              </span>
            </div>
