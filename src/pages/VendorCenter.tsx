...
type VendorType = 'entra_id' | 'okta' | 'keycloak' | 'cyberark' | 'ping_identity'
          ...
          <div className="bg-bg-card border border-border-subtle rounded-xl p-4 shadow-md">
            ...
            <div className="flex flex-col gap-2">
              {(Object.keys(VENDOR_DATA) as VendorType[]).map((key) => (
                ...
                >
                  <span>{key === 'entra_id' ? 'Entra ID' : key === 'okta' ? 'Okta' : key === 'keycloak' ? 'Keycloak (OS)' : key === 'cyberark' ? 'CyberArk' : 'Ping Identity'}</span>
