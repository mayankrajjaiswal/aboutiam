...
test.describe('Advanced Horizon 3 Next-Gen Playgrounds', () => {
  ...
  test('should run OPA Wasm Playground Simulator', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Wasm-Native OPA & Directory Engine Simulator")')).toBeVisible()
    ...
    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('allow = true')
  })
  ...
  test('should run MCP Server Playground', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Model Context Protocol (MCP) Server Sandbox")')).toBeVisible()

    const executeBtn = page.locator('button:has-text("Execute MCP Tool Protocol Handshake")')
    await expect(executeBtn).toBeVisible()
    await executeBtn.click()

    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('get_encyclopedia_term')
  })
  ...
  test('should run WebRTC P2P Playground', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("WebRTC P2P Cryptographic Handshake")')).toBeVisible()
    ...
    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('SDP Offer', { timeout: 5000 })
  })
  ...
  test('should run SOC War Room Simulator', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("War Room")')).toBeVisible()

    const rotateBtn = page.locator('button:has-text("Rotate Cloud Credentials")')
    await expect(rotateBtn).toBeVisible()
    await rotateBtn.click()

    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('rotated')
  })
  ...
  test('should run Biometric Mesh Playground', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Computer-Vision Biometric Mesh Lab")')).toBeVisible()

    const blinkBtn = page.locator('button:has-text("Simulate Blink Reflex")')
    await expect(blinkBtn).toBeVisible()
    await blinkBtn.click()

    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('genuine muscle reflex')
  })
})
