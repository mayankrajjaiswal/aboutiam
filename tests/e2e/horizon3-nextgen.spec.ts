...
test.describe('Horizon 3 Next-Gen Playgrounds and Tools', () => {
  ...
  test('should run RAG-Aware Authorization Simulator', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("RAG-Aware Authorization Policy Engine")')).toBeVisible()

    const triggerBtn = page.locator('button:has-text("CEO salary")').first()
    await expect(triggerBtn).toBeVisible()
    await triggerBtn.click()

    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('CEO Salary')
  })
  ...
  test('should run AI Swarm Orchestrator Simulator', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Ephemeral AI Swarm Identity Orchestrator")')).toBeVisible()

    const spawnBtn = page.locator('button:has-text("Spawn Constrained Swarm")')
    await expect(spawnBtn).toBeVisible()
    await spawnBtn.click()

    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('Child Swarm active')
  })

  // 3. Fully Homomorphic Encryption (FHE)
  test('should run FHE Auth Sandbox', async ({ page }) => {
    await page.goto('http://localhost:5173/playground/fhe-auth')
    await expect(page.locator('h1:has-text("Fully Homomorphic Encryption")')).toBeVisible()
    ...
    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('Polynomial intersection confirmed')
  })

  // 4. Quantum Key Distribution (QKD)
  test('should run QKD Simulator', async ({ page }) => {
    await page.goto('http://localhost:5173/playground/qkd-simulator')
    await expect(page.locator('h1:has-text("Quantum Key Distribution")')).toBeVisible()

    const qkdBtn = page.locator('button:has-text("Start QKD Handshake")')
    await expect(qkdBtn).toBeVisible()
    await qkdBtn.click()

    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('cryptographic key established')
  })

  // 5. mDL Proximity
  test('should run mDL Proximity Lab', async ({ page }) => {
    await page.goto('http://localhost:5173/playground/mdl-proximity')
    await expect(page.locator('h1:has-text("mDL Proximity Authentication Lab")')).toBeVisible()

    const tapBtn = page.locator('button:has-text("Tap mDL Phone")')
    await expect(tapBtn).toBeVisible()
    await tapBtn.click()

    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('age-attestation without full PII')
  })
  ...
  test('should run Space Identity DTN Simulator', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Space Identity & DTN Simulator")')).toBeVisible()

    const txBtn = page.locator('button:has-text("Transmit Space Bundle")')
    await expect(txBtn).toBeVisible()
    await txBtn.click()

    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('Lunar Relay Gateway', { timeout: 5000 })
  })

  // 7. V2X PKI
  test('should run V2X PKI Simulator', async ({ page }) => {
    await page.goto('http://localhost:5173/playground/v2x-pki')
    await expect(page.locator('h1:has-text("V2X PKI Expressway Simulator")')).toBeVisible()

    const verifyBtn = page.locator('button:has-text("Verify Compliant Vehicle")')
    await expect(verifyBtn).toBeVisible()
    await verifyBtn.click()

    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('verified leaf signature')
  })
  ...
  test('should run eBPF Kernel Identity Tracer', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("eBPF Kernel-Level Identity Tracer")')).toBeVisible()

    const executeBtn = page.locator('button:has-text("Execute Signed Binary")')
    await expect(executeBtn).toBeVisible()
    await executeBtn.click()

    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('probe intercepting sys_enter_connect')
  })
  ...
  test('should run Digital Twin Binding Workbench', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Digital Twin Identity Binding Workbench")')).toBeVisible()

    const bindBtn = page.locator('button:has-text("Bind PUF")')
    await expect(bindBtn).toBeVisible()
    await bindBtn.click()

    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('Physical Unclonable Function')
  })

  // 10. BCI Neural Auth
  test('should run BCI Neural Auth Simulator', async ({ page }) => {
    await page.goto('http://localhost:5173/playground/bci-auth')
    await expect(page.locator('h1:has-text("BCI Neural Auth Baseline Simulator")')).toBeVisible()

    const scanBtn = page.locator('button:has-text("Calibrated Baseline")')
    await expect(scanBtn).toBeVisible()
    await scanBtn.click()

    await expect(page.locator('div.terminal-high-contrast').first()).toContainText('P300 polynomial spikes')
  })
  ...
  test('should run C2PA Provenance Tool', async ({ page }) => {
    ...
    // Tool pages render their title as <h2> (they don't use PlaygroundShell's <h1>)
    await expect(page.locator('h2').first()).toContainText('C2PA Cryptographic Provenance Tool')

    const simulateBtn = page.locator('button:has-text("Hardware-Signed")')
    await expect(simulateBtn).toBeVisible()
    await simulateBtn.click()

    await expect(page.locator('p:has-text("Leica Camera AG")').first()).toBeVisible()
  })
  ...
  test('should run EU AI Act Compliance Assessor Tool', async ({ page }) => {
    ...
    await expect(page.locator('h2').first()).toContainText('EU AI Act Identity Compliance Assessor')
  ...
  test('should run Log Anonymizer Tool', async ({ page }) => {
    ...
    await expect(page.locator('h2').first()).toContainText('OIDC / SAML Trace Log Anonymizer')
